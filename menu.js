...
const mealContainers = document.querySelector('.mealContainers');


//Event Listeners
window.addEventListener('DOMContentLoaded', function(){
    let displayMenu = menu.map(function(menu){
        return `<section class="menuItems">
        <img src= ${menu.img} alt="${menu.title}"></img>
        <h4 id="mealName">${menu.category} <span id="price">${menu.price}</span></h4>
        <span id="borderBottomMenu"></span>
        <p>${menu.desc}
    </section>`
    });
    displayMenu = displayMenu.join('');
    mealContainers.innerHTML = displayMenu
})